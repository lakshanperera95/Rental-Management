        ...
        private void btnSupplierSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (txtSuppCode.Text.Trim() == string.Empty && txtSuppName.Text.Trim() == string.Empty)
                    {
                        ObjPaymentSetOff.SqlStatement = "SELECT Cust_Code AS [Customer Code],Cust_Name AS [Customer Name] FROM Customer " + Loca + " ORDER BY Cust_Code";
                    }
                    else
                    {
                        if (txtSuppCode.Text.Trim() != string.Empty && txtSuppName.Text.Trim() == string.Empty)
                        {
                            ObjPaymentSetOff.SqlStatement = "SELECT Cust_Code AS [Customer Code],Cust_Name AS [Customer Name] FROM Customer WHERE Cust_Code LIKE '%" + txtSuppCode.Text.Trim() + "%' " + Loca + " ORDER BY Cust_Code";
                        }
                        else
                        {
                            if (txtSuppCode.Text.Trim() == string.Empty && txtSuppName.Text.Trim() != string.Empty)
                            {
                                ObjPaymentSetOff.SqlStatement = " SELECT Cust_Code AS [Customer Code],Cust_Name AS [Customer Name] FROM Customer WHERE Cust_Name LIKE '%" + txtSuppName.Text.Trim() + "%' " + Loca + " ORDER BY Cust_Name";
                            }
                            else
                            {
                                ObjPaymentSetOff.SqlStatement = "SELECT Cust_Code AS [Customer Code],Cust_Name AS [Customer Name] FROM Customer " + Loca + " ORDER BY Cust_Code";
