...
namespace Inventory
{
    public partial class frmSupplierPayment : Form
    {
            ...
            try
            {
                if (e.KeyCode == Keys.Enter && txtSuppCode.Text.Trim() != "")
                {
                    ...
                    if (objSupplierPayment.RecordFound == true)
                    {
                        ...
                        objSupplierPayment.SqlStatement = "SELECT  * FROM dbo.Payment_Summary  WHERE Tr_Type IN ('SRN','SADV','OVPMT') AND Balance_Amount>0 AND Loca ='" + LoginManager.LocaCode + "' AND  Acc_No='" + txtSuppCode.Text.Trim() + "'";
                        DataSet Ds = objSupplierPayment.GetDataset();
        ...
        private void lblSetOff_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
        ...
        private void toolStripMenuItem_OpenDoc_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objSupplierPayment.SqlStatement = "SELECT Supp_Code, Supp_Name FROM Supplier WHERE Supp_Code = '" + txtSuppCode.Text.Trim() + "'";
                objSupplierPayment.ReadSupplierDetails();
                if (objSupplierPayment.RecordFound == false)
                {
                   MessageBox.Show("Supplier Code Not Found.", "Receipt", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    txtSuppCode.Focus();
                }

                if (frmCreditNote.objDebNote != null)
                    frmCreditNote.objDebNote.Close();
                ...
                else
                {
                    ...
                    frmCreditNote.objDebNote = new frmCreditNote(2, txtSuppCode.Text, DocNo, lblTempDocNo.Text);
                    //frmCreditNote.objDebNote.MdiParent = MainClass.mdi;
                    frmCreditNote.objDebNote.ShowDialog();
