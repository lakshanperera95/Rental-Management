            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    if (e.KeyCode == Keys.Enter && txtQty.Text.Trim() != string.Empty)
                    {
                        ...
                        {
                            objGrn.SqlStatement = "SELECT product.Prod_Code, product.Prod_Name, product.Purchase_price, CAST(product.Selling_Price AS DECIMAL(18,2))[Selling_Price], Stock_Master.Qty, product.Pack_Size, product.Unit, CAST(Product.Marked_Price AS DECIMAL(18,2)) [Marked_Price], CAST(Product.Whole_Price AS DECIMAL(18,2))[Whole_Price] FROM product INNER JOIN Stock_Master ON product.Prod_Code = Stock_Master.Prod_Code WHERE product.Supplier_Id = '" + txtSuppCode.Text.Trim() + "' AND product.Prod_Code = '" + txtProductCode.Text.Trim() + "' AND LockedItem='F' and Stock_Master.Loca = ";
                        }
                        else
                        {
