    ...
    public class clsProductPriceMaste
    {
        ...
        public void GetTempDatasetPriceBatchWise()
        {
            try
            {
                ...
                dsTempPriceLevelDet = dbcon.getDataset("SELECT Prod_Code, Selling_Price [RPrice], Whole_Price [WPrice], Price_Level [DPrice], Marked_Price [MMPrice], Purchase_Price from tbTempPriceLevel WHERE Prod_Code = '" + strProductCode + "'   order by  Ln ", "dsPriceMasterDetails");
            }
            catch (Exception e)
            {
                //Write to Log
