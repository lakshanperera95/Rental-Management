    ...
    public class clsSupplierReturn : clsPublic
    {
        ...
        public void GetTempDataset()
        {
            try
            {
                ...
                dsTempGrn = dbcon.getDataset("select Prod_Code, Prod_Name, Unit, Purchase_Price, Selling_Price, Qty, FreeQty, Discount,Amount, ExpDate, BatchNo FROM TransactionTemp_Details WHERE Doc_No = '" + strTempDocumentNo + "' AND IId = 'SRN' AND Loca = " + LoginManager.LocaCode + " Order by Ln", "GoodReceivedNote");
            }
            catch (Exception e)
            {
                //Write to Log
