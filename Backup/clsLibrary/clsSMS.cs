...
namespace clsLibrary
{
    public class clsSMS
    {
        ...
        decimal _strLoginUser;
        public decimal decLen
        {
            get { return _strLoginUser; }
            set { _strLoginUser = value; }
        }
        bool _decGenCod;
        ...
        public static void ErrMessage(string error, string errorLocation)
        {
            ...
            try
            {
               m_streamWriter.WriteLine("Date/Time :' " + string.Format("{0:dd/MM/yyyy HH:mm:ss}", dtCurrentDate) + "'Error Location  : " + errorLocation + ".   Error Description  :" + error.ToString() + " 'Location :'" + LoginLocaCode.Trim() + "'User Name :'" + LoginUser.Trim() + "'");
            }
            finally
            {
