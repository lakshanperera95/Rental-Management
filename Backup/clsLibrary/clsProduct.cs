    ...
    public class clsProduct:clsPublic
    {
        ...
        public void UpdateData()
        {
            try
            {
                ...
                command.Parameters.Add(new SqlParameter("@Disc_Price", SqlDbType.Money, 0, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, DiscountPrice));
...
//                    intPurchasePrice = int.Parse( strPurchasePrice);
        ...
        public void GetTempDataset()
        {
            try
            {
                ...
                dsLocationROL = dbcon.getDataset("SELECT SM.Prod_Code, SM.ROL, SM.RO_Qty, SM.Loca, L.Loca_Descrip, SM.Qty FROM Location L INNER JOIN Stock_Master SM ON L.Loca = SM.Loca WHERE SM.Prod_Code = '" + strCode + "' Order by SM.Loca", "dsProdROL");
            }
            catch (Exception e)
            {
                //Write to Log
