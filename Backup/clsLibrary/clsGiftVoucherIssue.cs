...
namespace clsLibrary
{
    public class clsGiftVoucherIssue
    {
       ...
       DataSet  _Ds;
        ...
        public void UpdateGIIssue(string DocNo, string Code)
        {
            try
            {
                Sqlst = "EXEC dbo.gif_sp_VoucherIssueTEmpUpdate  @DocNO = @DocNO,@GiftNo = @GiftNo,@Loca = @Loca, @Iid = @Iid";
        ...
        public DataSet LoadData(string DocNo)
        {
            try
            {
                Sqlst = "SELECT G.BookNo[Doc_no],Code,CAST( TD.Amount AS DECIMAL(18,2))[Amount] FROM dbo.TransactionTemp_Details TD JOIN dbo.gif_Voucher G ON G.VisibleCode=TD.Code WHERE TD.Doc_No=@DocNO AND TD.Loca=@Loca AND TD.Iid=@Iid;SELECT CAST( ISNULL(SUM(Amount),0) AS DECIMAL(18,2)) FROM dbo.TransactionTemp_Details WHERE Doc_No=@DocNO AND Loca=@Loca AND Iid=@Iid;";
                 List<SqlParameter> lstPrm = new List<SqlParameter>();
                 lstPrm.Add(new SqlParameter("@DocNO", DocNo));
                 lstPrm.Add(new SqlParameter("@Loca", LoginManager.LocaCode));
                 lstPrm.Add(new SqlParameter("@Iid", "GVI"));
        ...
        public DataSet   ApplyData(string DocNo,string Date,string CustCode)
        {
            try
            {
                Sqlst = "EXEC gif_sp_VoucherIssueApply @DocNo=@DocNO, @Loca=@Loca, @Iid=@Iid,@Date=@Date,@CustCode=@CustCode,@CreateUser=@CreateUser ;";
                List<SqlParameter> lstPrm = new List<SqlParameter>();
                lstPrm.Add(new SqlParameter("@DocNO", DocNo));
                lstPrm.Add(new SqlParameter("@Loca", LoginManager.LocaCode));
                lstPrm.Add(new SqlParameter("@Iid", "GVI"));
