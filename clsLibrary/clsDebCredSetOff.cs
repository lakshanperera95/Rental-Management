    ...
    public class clsDebCredSetOff
    {
        ...
        public void GetTempDocumentNo()
        {
            ...
            try
            {
                ...
                DataSet dsD = dbcon.getDataset(strSqlStatement + LoginManager.LocaCode, "DsDocNo");
                if (dsD.Tables["DsDocNo"].Rows.Count==1)
                {
                    intTempDocNo = int.Parse(dsD.Tables["DsDocNo"].Rows[0][0].ToString());
                    //String.Format("{0:00000}", 15); 
                    strTempDocumentNo = string.Format("{0:000000}", intTempDocNo);
                    strTempDocumentNo = "DCS" + LoginManager.LocaCode.Trim().Substring(0, 2) + strTempDocumentNo;
            ...
            try
            {
                ...
                command.Parameters.Add(new SqlParameter("@Iid", SqlDbType.NVarChar, 5, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, "DCS"));
        ...
        public void GetTempDataset()
        {
            try
            {
                ...
                dsPendingPayment = dbcon.getDataset("SELECT Doc_No, Transaction_Date, Transaction_Amount, Balance_Amount, Temp_DocNo FROM tbPendingPayments_CRD WHERE Balance_Amount > 0 AND Temp_DocNo = '" + strTempDocumentNo + "' AND Loca = " + LoginManager.LocaCode + "", "PendingPayments");
                dsSupplierReturn = dbcon.getDataset("SELECT Doc_No, Transaction_Date, Transaction_Amount, Balance_Amount, Temp_DocNo FROM tbPendingPayments_DEB WHERE Balance_Amount > 0 AND Temp_DocNo = '" + strTempDocumentNo + "' AND Loca = " + LoginManager.LocaCode + "", "PendingReturns");
                dsSelectedsSetOff = dbcon.getDataset("SELECT Grn_Doc, Grn_Tr_Date, Grn_Tr_Amount, Grn_Balance_Amount, Srn_Doc, Srn_Tr_Date, Srn_Tr_Amount ,Srn_Balance_Amount, Temp_DocNo, Loca FROM tbTempSetoff WHERE Temp_DocNo = '" + strTempDocumentNo + "' AND IId = 'DCS' AND Loca = " + LoginManager.LocaCode + " ", "SelectedSetoff");
            }
            catch (Exception e)
            {
                //Write to Log
        ...
        public void LoadSupplierTransaction()
        {
            try
            {
                ...
                command.Parameters.Clear();
                command.Parameters.Add(new SqlParameter("@Err_x", SqlDbType.Int, 0, ParameterDirection.InputOutput, false, 0, 0, "", DataRowVersion.Default, intErrCode));
                command.Parameters.Add(new SqlParameter("@Iid", SqlDbType.NVarChar, 5, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, "DCS-C"));
        ...
        public void LoadCustomerTransaction()
        {
            try
            {
                ...
                command.Parameters.Clear();
                command.Parameters.Add(new SqlParameter("@Err_x", SqlDbType.Int, 0, ParameterDirection.InputOutput, false, 0, 0, "", DataRowVersion.Default, intErrCode));
                command.Parameters.Add(new SqlParameter("@Iid", SqlDbType.NVarChar, 5, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, "DCS-D"));
                command.Parameters.Add(new SqlParameter("@Loca", SqlDbType.NVarChar, 5, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, LoginManager.LocaCode));
        ...
        public void GetOrgDocumentNo()
        {
            try
            {
                DataSet dsDO = dbcon.getDataset("select DebCredSetOf from DocumentNoDetails WHERE Loca = '" + LoginManager.LocaCode + "'", "dsOrgDoc");
                if (dsDO.Tables["dsOrgDoc"].Rows.Count==1)
                {
                    intTempDocNo = int.Parse(dsDO.Tables["dsOrgDoc"].Rows[0][0].ToString());
                    //String.Format("{0:00000}", 15); 
                    strOrgDocNo = string.Format("{0:000000}", intTempDocNo);
                    strOrgDocNo = "DCS" + LoginManager.LocaCode.Trim().Substring(0, 2) + strOrgDocNo;
        ...
        public void SetOffApply()
        {
            ...
            try
            {
                ...
                command.Parameters.Add(new SqlParameter("@Acc_Code2", SqlDbType.NVarChar, 15, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, strCustCode));
                command.Parameters.Add(new SqlParameter("@Loca", SqlDbType.NVarChar, 5, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, LoginManager.LocaCode));
                command.Parameters.Add(new SqlParameter("@Iid", SqlDbType.NVarChar, 5, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, "DCS"));
        ...
        public void GetDataSetForReport()
        {
            try
            {
                dsForReport = dbcon.getDataset("SELECT PD.Org_Doc_no, PD.Doc_No, PD.Transaction_Date, PD.Transaction_Amount, PD.Paid_Amount, PD.Post_Date, PD.SetOff_SR_Doc, PD.Loca, L.Loca_Descrip, PD.Acc_Code, S.Supp_Name, PD.SetOff_Acc_Code , C.Cust_Name, 'Original' Status, PD.SetOff_Doc_Date AS Transaction_Date2 FROM tbPaidPaymentDetails PD INNER JOIN Location L ON PD.Loca = L.Loca INNER JOIN Supplier S ON PD.Acc_Code = S.Supp_Code INNER JOIN Customer C ON PD.SetOff_Acc_Code=C.Cust_Code WHERE PD.Org_Doc_no = '" + strOrgDocNo + "' AND PD.Loca = '" + LoginManager.LocaCode + "' AND PD.Iid = 'DCS'", "dsPaymentsetOffDetails");
            }
            catch (Exception e)
            {
                //Write to Log
