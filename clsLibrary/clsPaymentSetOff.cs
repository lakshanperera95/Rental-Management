...
namespace clsLibrary
{
    public class clsPaymentSetOff
    {
        ...
        public void GetTempCustomerDataset()
        {
            ...
            {
                //get Temporary DataSet For Grn
                dsPendingPayment = dbcon.getDataset("SELECT Doc_No, Transaction_Date, Transaction_Amount, Balance_Amount, Temp_DocNo FROM tbPendingPayments WHERE Balance_Amount > 0 AND Temp_DocNo = '" + strTempDocumentNo + "' AND Tr_Type in ('INV','CQR','RRT') AND IId = 'SOR' AND Loca = '" + LoginManager.LocaCode + "'", "PendingPayments");
                dsSupplierReturn = dbcon.getDataset("SELECT Doc_No, Transaction_Date, Transaction_Amount, Balance_Amount, Temp_DocNo FROM tbPendingPayments WHERE Balance_Amount > 0 AND Temp_DocNo = '" + strTempDocumentNo + "' AND Tr_Type in ('CADV','CUR','OVREC') AND IId = 'SOR' AND Loca = '" + LoginManager.LocaCode + "'", "PendingReturns");
