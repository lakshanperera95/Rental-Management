...
namespace clsLibrary
{
    public class clsRent
    {
        ...
        public string  SalesAssisCode { get; set; }
        ...
        public void GetTempDataset()
        {
            try
            {
                ...
                dsTempInvoice = dbcon.getDataset("select Prod_Code, Prod_Name, Unit, selling_price as Purchase_Price, Selling_Price, Qty, FreeQty, Discount,Amount  FROM Transaction_Details WHERE Doc_No = '" + strTempDocumentNo + "' AND IId = 'INV'  and Transaction_Details.InvType = 'RENT' and Transaction_Details.Rrecall = 'F' AND Loca = " + LoginManager.LocaCode + " Order by Ln", "Invoice");
            }
            catch (Exception e)
            {
                //Write to Log
        ...
        public void ReadAssitantDetail()
        {
            try
            {
                DataReader = dbcon.GetReader(strSqlStatement);
                ...
                if (DataReader.Read())
                {
                    SalesAssisCode = DataReader["Sale_Code"].ToString().Trim();
                    SalesAssistant = DataReader["Sale_Name"].ToString().Trim();                
                    blRecordFound = true;
                }
                else
                {
                    SalesAssisCode = string.Empty;
