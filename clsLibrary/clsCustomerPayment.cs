    ...
    public class clsCustomerPayment
    {
        ...
        public void GetBankDetails()
        {
            try
            {
                ...
                dsBankDetails = dbcon.getDataset("SELECT * FROM BankDetails ORDER BY Bank_Code;SELECT Ctype,Name   FROM dbo.Pay_Settings WHERE FType='RECPAY';SELECT Name   FROM dbo.Pay_Settings WHERE FType='CTYPE' ", "BankDetails");
            }
            catch (Exception e)
            {
                //Write to Log
        ...
        public void GetTempDataset()
        {
            try
            {
                ...
                dsPendingPayment = dbcon.getDataset("select Doc_No, Transaction_Date, Transaction_Amount, Balance_Amount, Temp_DocNo from tbPendingPayments WHERE Temp_DocNo = '" + strTempDocumentNo + "' AND IId = 'REC' AND Loca = '" + LoginManager.LocaCode + "' ORDER BY CONVERT(DATETIME,Transaction_Date,103), RIGHT(DOC_NO,6) ", "PendingPayments");
        ...
        public void LoadCustomerTransaction(bool Tax)
        {
            try
            {
                //UpDate Record No
                ...
                command.Parameters.Add(new SqlParameter("@Tax", SqlDbType.Bit, 11, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, Tax));
        ...
        public void GetOrgDocumentNo(bool Tax)
        {
            try
            {
                string Loca = LoginManager.LocaCode;
                if (Tax)
                {
                    Loca = LoginManager.TaxLoca;
                }

                DataReader = dbcon.GetReader("select Receipt_No from DocumentNoDetails WHERE Loca = " + Loca);
                if (DataReader.Read())
                {
                    intTempDocNo = (int)DataReader.GetSqlInt32(0);
                    //String.Format("{0:00000}", 15); 
                    strOrgDocNo = string.Format("{0:000000}", intTempDocNo);                    
                    strOrgDocNo = "REC" + Loca.Trim().Substring(0, 2) + strOrgDocNo;
        ...
        public void CustomerPaymentApply(bool Tax)
        {
            DateTime PostDate = DateTime.Now;
            try
            {
              ...
              //  strPostDate = string.Format("{0:dd/MM/yyyy}", PostDate);
                ...
                command.Parameters.Add(new SqlParameter("@Unit", SqlDbType.Int, 50, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, LoginManager.MachineUnit));
                command.Parameters.Add(new SqlParameter("@Tax", SqlDbType.Bit, 50, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, Tax));
        ...
        public void GetDataSetForReport(bool sinhala)
        {
            try
            {
                if (sinhala)
                {
                    dsForReport = dbcon.getDataset("SELECT tbPaidPaymentDetails.Org_Doc_no, tbPaidPaymentDetails.Post_Date, tbPaidPaymentDetails.Doc_No, tbPaidPaymentDetails.Transaction_Date, tbPaidPaymentDetails.Balance_Amount, tbPaidPaymentDetails.Transaction_Amount, tbPaidPaymentDetails.Paid_Amount, tbPaidPaymentDetails.Loca, Location.Loca_Descrip, tbPaidPaymentDetails.Acc_Code, customer.cust_Name Acc_Name, customer.Address1, customer.Address2, customer.Address3, customer.Address4, tbPaidPaymentDetails.[User_Name], 'Original' Status,PreBalance[Balance] FROM tbPaidPaymentDetails INNER JOIN customer ON tbPaidPaymentDetails.Acc_Code = Customer.Cust_Code INNER JOIN Location ON tbPaidPaymentDetails.Loca = Location.Loca WHERE tbPaidPaymentDetails.Org_Doc_no = '" + strOrgDocNo + "' AND tbPaidPaymentDetails.Loca = '" + LoginManager.LocaCode + "' AND tbPaidPaymentDetails.Iid = 'REC';SELECT * FROM CompanyProfileSinhala", "tbPaidPaymentSummary");
                }
                else
                {
