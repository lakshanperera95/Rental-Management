...
namespace clsLibrary
{
    public class clsBundleIssue
    {
        ...
        public void UpdateTempBundlingIssuePacketing(string ProdCode,decimal Qty,string BatchNo,string DocNo,string MainCode,decimal NoOfPkt)
        {
            try
            {
                intErrCode = 0;
                ...
                command.Parameters.Add(new SqlParameter("@MainCode", SqlDbType.NVarChar, 25, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, MainCode));
                ...
                command.Parameters.Add(new SqlParameter("@Temp_DocNo", SqlDbType.NVarChar, 20, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, DocNo));
                command.Parameters.Add(new SqlParameter("@No_Of_Packets", SqlDbType.Decimal, 0, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Default, NoOfPkt));
        ...
        public void DeleteTempBundlingProduct(string ProdCode, string DocNo,string BatchNo)
        {
            try
            {
                intErrCode = 0;
        ...
        public void IssuingBunlingProductApply()
        {
            ...
            try
            {
              ...
              ///  strPostDate = string.Format("{0:dd/MM/yyyy}", PostDate);
                ...
                command.Parameters.Add(new SqlParameter("@OrgDoc_No", strOrgDocNo ));
        ...
        public void LoadTempDetails(decimal Qty,string DocNo)
        {
            try
            {
                strSqlStatement = "SELECT TM.Prod_Code[Main_Prod_Code],P.Prod_Name[Main_Prod_Name],CAST( TM.Purchase_price AS DECIMAL(18,2))Purchase_price,CAST(TM.No_Of_Packets AS DECIMAL(18,2))No_Of_Packets,CAST( TM.Purchase_price*TM.No_Of_Packets AS DECIMAL(18,2))[Packet_Qty] ,TM.BatchNo FROM dbo.tbTempBundlingIssueProduct TM JOIN dbo.Product P ON TM.Prod_Code=P.Prod_Code WHERE TempDocNo='" + DocNo+ "';SELECT  isnull(CAST( SUM(TM.Purchase_price*TM.No_Of_Packets) AS DECIMAL(18,2)),0) , isnull(CAST( (SUM((TM.Purchase_price*TM.No_Of_Packets)) / " + Qty+ ") AS DECIMAL(18,2)),0)  , isnull(CAST( P.Selling_Price AS DECIMAL(18,2)),0) FROM dbo.tbTempBundlingIssueProduct TM JOIN dbo.Product P ON TM.Main_Prod_Code=P.Prod_Code WHERE TM.TempDocNo='" + DocNo+"' GROUP BY  P.Selling_Price";

                dsTempSelectedItem = dbcon.getDataset(strSqlStatement, "ds");
